...
from . import xsys35
from .model import escape_text, parse_edit_text
...
def _byte_preserving_edit(edit_text: str, encoding: str) -> str:
    """Transport target bytes as Latin-1 via UTF-8 text assignments.

    alice 0.13.0 initializes member functions using its output encoding before
    applying edits. Latin-1 accepts mixed-encoding names without changing their
    stored bytes. Encode each replacement strictly first, then escape the byte
    representation (including multibyte trail bytes equal to backslash).
    Only numeric assignments generated by this application are supported.
    """
    lines = []
    try:
    ...
    for (kind, index), text in assignments.items():
        try:
            raw = text.encode(encoding)
        except (UnicodeError, LookupError) as exc:
            raise AliceError(f"文本 {kind}[{index}] 无法用 {encoding} 编码写回") from exc
        if b"\0" in raw:
            raise AliceError("写回文本不能包含 NUL 字节")
        transport = raw.decode("latin1")
        # The upstream text lexer uses a fixed 65536-byte string buffer.
        if len(transport.encode("utf-8")) >= 65536:
            raise AliceError(f"文本 {kind}[{index}] 超出 alice 单条文本长度限制")
        lines.append(f'{kind}[{index}] = "{escape_text(transport)}"')
    return "\n".join(lines) + "\n"
        ...
        with tempfile.TemporaryDirectory(prefix=".aindiff-edit-", dir=ain_path.parent) as tmp:
            ...
            try:
                ...
                try:
                    ...
                    if "iconv:" not in str(exc):
                        raise
                    edit_file.write_text(
                        _byte_preserving_edit(edit_text_data, output_encoding), encoding="utf-8"
                    )
                    # A failed attempt must never supply the retry's output.
                    if out_file.exists():
                        out_file.unlink()
                    args[args.index("--output-encoding") + 1] = "ISO-8859-1"
                    self.run(args, timeout=timeout)
